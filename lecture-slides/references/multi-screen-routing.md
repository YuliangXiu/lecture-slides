...
6. **默认打开的 deck 是 `/03-slides/session-1/`，不是 `/04-deploy/`**（2026-09-09 起，用户明确要求）。单屏回退分支同样开 02-script + 03-slides/session-1（不注入 pv*）。改 play.command 时两份（lecture-01/02）要一起对齐。
