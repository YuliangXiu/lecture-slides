...
1. **规则必须按「长/具体 → 短/泛化」排序**，否则短规则先吃掉长规则的匹配。典型：绝对路径 `/Users/<user>/…/<lecture>` 必须在 `/Users/<user>`、`<lecture>` 之前替换，否则会拼出 `/Users/<作者>` 这类半截占位符。
2. **依赖「另一条规则的产物」的规则要放最后，或另开后置阶段**。实例：想让 `…/_shared/` 收敛成 `…/_shared/`，这条必须在 `… Course → <课程目录>` 之后跑；同表但顺序靠前就永远不命中。
