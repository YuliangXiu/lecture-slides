...
| 项 | 值 |
|---|---|
| 存储桶 | `<COS_BUCKET>`（AppID 已含于桶名，配置时勿重复附加） |
| 区域 | `<COS_REGION>` |
| COS 前缀 | `/<COURSE_ID>/media/decks/` |
| 本地源 | `<课程根>/04-deploy/media/decks/` |
| 基础 URL | `https://<COS_BUCKET>.cos.<COS_REGION>.myqcloud.com` |

路径映射：本地 `04-deploy/media/decks/session-N/videos/x.mp4` → COS Key `<COURSE_ID>/media/decks/session-N/videos/x.mp4`。

学生版 HTML（`build_publish.py` 产物）把 `../media/decks/` 替换为 `https://<COS_BUCKET>.cos.<COS_REGION>.myqcloud.com/<COURSE_ID>/media/decks/`，所以 COS 上的 Key 必须与本地相对路径**严格一致**。

## 2. 首选工具：rclone

`~/.rclone.conf` 配好 `cos` remote（provider=TencentCOS，权限 600）；作者实例实测 `rclone copy` 420MB/49s，量级供参考。未配置见 2.1。

```bash
# 验证 remote 已配
/opt/homebrew/bin/rclone listremotes      # 应含 cos:
/opt/homebrew/bin/rclone lsd cos:<COS_BUCKET>
...
- **必须用全路径 `/opt/homebrew/bin/rclone`**：新开 shell 的 PATH 不含 homebrew，裸 `rclone` 会找不到命令（`~/.rclone.conf` 的 `cos` remote 配好后，`/opt/homebrew/bin/rclone listremotes` 应含 `cos:`）。
...
- 核对：`/opt/homebrew/bin/rclone lsf cos:<COS_BUCKET>/<COURSE_ID>/media/decks --include '*.webp' | wc -l` 与本地 webp 数对比，确认新格式已全量就位。
...
> 腾讯云 COS 兼容 S3 协议，rclone 用 `s3` 类型 + TencentCOS provider 即可；remote 名统一叫 `cos`，桶名 `<COS_BUCKET>`。
...
验证：`coscmd --version`（venv 内为 `<venv>/bin/coscmd --version`）。

### 3.2 配置（密钥写入 `~/.cos.conf`）

```bash
coscmd config -a <SecretId> -s <SecretKey> -b <COS_BUCKET> -r <COS_REGION>
```
...
```bash
coscmd upload -r --skipmd5 <课程根>/04-deploy/media/decks/ /<COURSE_ID>/media/decks/
```
...
- 目标前缀**必须带前导 `/`**（相对桶根）。
- 校验：上传后 `coscmd list -r /<COURSE_ID>/media/decks/ | wc -l` 与本地文件数一致；抽查 `coscmd info /<COURSE_ID>/media/decks/<某文件>` 返回 200。

### 3.4 常用维护命令

```bash
coscmd list -r /<COURSE_ID>/media/decks/        # 列出前缀下所有对象
coscmd info /<COURSE_ID>/media/decks/<key>      # 查单个对象头
coscmd delete -r -f /<COURSE_ID>/media/decks/xxx  # 删（慎用，勿带 -f 盲删根前缀）
```
...
COS_SECRET_ID=xxx COS_SECRET_KEY=yyy python3 <本 skill 目录>/scripts/cos_sync.py \
    --src <课程根>/04-deploy/media/decks --prefix /<COURSE_ID>/media/decks --apply
```
...
4. 团队共享：每人用自己的子账号密钥，最小权限（仅 `<COS_BUCKET>` 桶的读写）。
...
| 桶/区域写错 | 404 / NoSuchBucket | 核对桶名 `<COS_BUCKET>`、区域 `<COS_REGION>` |
